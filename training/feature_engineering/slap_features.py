...
def generate_slap_features():
    # -------------------------------
    # Load base SLAP node features
    # -------------------------------
    sku_nodes = pd.read_csv(DATA_PATH + "sku_node_features.csv")
    bin_nodes = pd.read_csv(DATA_PATH + "bin_node_features.csv")

    # -------------------------------
    # Load GMM and ABC results
    # -------------------------------
    gmm_clusters = pd.read_csv(DATA_PATH + "gmm_features.csv")
    abc_results = pd.read_csv(DATA_PATH + "abc_results.csv")
    ...
    # -------------------------------
    sku_nodes.to_csv(DATA_PATH + "sku_node_features_scaled.csv", index=False)
    bin_nodes.to_csv(DATA_PATH + "bin_node_features_scaled.csv", index=False)

    print("SLAP GNN features prepared with GMM clustering and ABC priority")

if __name__ == "__main__":
    generate_slap_features()
